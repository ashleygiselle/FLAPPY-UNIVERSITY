...
var GRAVITY = 60;
var FLAP = 200;
var SPAWN_RATE = 1 / 1.1;
var OPENING_MIN = 175; // Apertura mínima
var OPENING_MAX = 285; // Apertura máxima

var GRAVITY_INCREMENT = 0.5;
var FLAP_INCREMENT = 0.5;
...
function preload() {
    var assets = {
        spritesheet: {
            birdie: ['assets/birdie1.1.png', 24, 24],
...
var gameStarted,
    ...
    instText,
    gameOverText,
    pauseText, // Variable para el texto de pausa
    flapSnd,
    scoreSnd,
    hurtSnd,
    fingersTimer,
    cloudsTimer,
    spaceKey,
    escapeKey; // Variable para la tecla de escape
...
function create() {
    ...
    escapeKey.onDown.add(togglePause);
...
function reset() {
    ...
    instText.setText("ESPACIO PARA EMPEZAR\nBUENA SUERTE");
    gameOverText.renderable = false;
    pauseText.visible = false; // Asegúrate de que el texto de pausa no sea visible al reiniciar
    ...
    fingers.removeAll();
    invs.removeAll();
    // Reinicia los valores de velocidad
    GRAVITY = 40;
    FLAP = 620;
    SPEED = 300;
}
...
function spawnFingers() {
    ...
    OPENING = Math.floor(Math.random() * (OPENING_MAX - OPENING_MIN + 1) + OPENING_MIN); // Genera apertura aleatoria
    var fingerY = ((game.height - 16 - OPENING) / 2) + (Math.random() > 0.5 ? -1 : 1) * Math.random() * game.height / 6;
    // Bottom finger
...
function addScore(_, inv) {
    ...
    // Increment gravity, flap, speed, and reduce opening gradually based on score
